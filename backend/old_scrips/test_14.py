...
def corrosion_analysis(data):
    ...
    for imagePath in os.listdir(imageFolder):
        ...
        cv2.imwrite(save_dir+'processed_1.png', image)
        ...
        cv2.imwrite(save_dir+'processed.png', mask)
        ...
        cv2.imwrite(save_dir+'processed_2.png', crop)
        ...
        cv2.imwrite(save_dir+'processed_3.png', s1)
        ...
        cv2.imwrite(save_dir+'processed_4.png', display)
        ...
        display2 = np.hstack([seg])

        im2 = Image.fromarray(display2.astype("uint8"))
        R, G, B = im2.split()
        im2 = Image.merge("RGB", (B, G, R))
        rawBytes = io.BytesIO()
        im2.save(rawBytes, "PNG")
        rawBytes.seek(0)  # return to the start of the file

        output2 = base64.b64encode(rawBytes.read()).decode('utf8')

        # passing the cropped image too
        display3 = np.hstack([edge_images])
        im3 = Image.fromarray(display3.astype("uint8"))
        #R, G, B = im3.split()
        #im3 = Image.merge("RGB", (B, G, R))
        rawBytes = io.BytesIO()
        im3.save(rawBytes, "PNG")
        rawBytes.seek(0)  # return to the start of the file
        edges = base64.b64encode(rawBytes.read()).decode('utf8')
        ...
        result["output_img"] = output2
        result["total_pixels"] = total_
        result["edges"] = edges
