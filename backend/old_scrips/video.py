...
m1 = tf.keras.models.load_model('C:/Users/patel/Desktop/p/model/unet/resnet101/new_unet_model_e400.h5',compile=False)
font = cv2.FONT_HERSHEY_SIMPLEX
...
def folder_process(original_dir):
    ...
    for file in os.listdir(original_dir):
        if file.endswith('.mp4'):
            ...
            mask_shape = (512,512)
            out_vid = cv2.VideoWriter(save_dir+'processed_1'+file,cv2.VideoWriter_fourcc('M','J','P','G'), fps, (2*frame_width,frame_height))
            ...
            while (vid.isOpened()):
                ...
                if ret == True:
                    ...
                    prediction1 = np.where(prediction < 0.1, 0, prediction)
                    prediction1 = np.where(prediction >= 0.1, 1, prediction)
                    ...
                    final = cv2.bitwise_and(frame, frame, mask=prediction5)
                      #find contours
                    contours, hierarchy = cv2.findContours(prediction5, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        ...
        if file.endswith('.png') or file.endswith('.jpg') or file.endswith('.jpeg'):
            ...
            output = cv2.putText(img, 'qualiTEAS', org, font,fontScale, color, thickness, cv2.LINE_AA)
...
if __name__ == '__main__':
    ...
    for line in data:
        ...
        inputData += line
    folder_process(json.loads(line))
