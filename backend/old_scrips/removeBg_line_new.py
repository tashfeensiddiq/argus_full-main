...
def removeBg_line(data):
    ...
    mask_original = cv2.imread(
        '/Users/sakura/Desktop/Argus 1.21/Argus_1.21_Back_end/bg_images/mask/' + tail, cv2.IMREAD_GRAYSCALE)
    # if (os.path.isfile('/Users/sakura/Desktop/Argus 1.21/Argus_1.21_Back_end/bg_images/mask/temp_' + tail)):

    #     mask_copy = cv2.resize(
    #         mask_copy, (img.shape[1], img.shape[0]))
    #     mask_flag0 = mask_copy
    # compare = mse(mask_original, mask_copy)
    # if compare == 0:
    #     mask_flag0 = mask_original
    # else:
    # mask_flag0 = mask_copy
    #     cv2.imwrite(save_dir+'/mask/org_'+tail, mask_original)
    if (os.path.isfile('/Users/sakura/Desktop/Argus 1.21/Argus_1.21_Back_end/bg_images/mask/temp_' + tail)) == False:
        img2 = img.copy()
        mask_flag0 = mask_original

        cv2.imwrite(save_dir+'/mask/temp_'+tail, mask_original)
        cv2.imwrite(save_dir+'/mask/org_'+tail, mask_original)
        mask_flag0 = cv2.resize(
            mask_flag0, (img.shape[1], img.shape[0]))
        output = cv2.bitwise_and(img2, img2, mask=mask_flag0)
        cv2.imwrite(save_dir+'/bg_path/org_'+tail, output)
    ...
    if flag == 0:
        ...
        mask = cv2.imread(
            '/Users/sakura/Desktop/Argus 1.21/Argus_1.21_Back_end/bg_images/mask/temp_' + tail)
        mask = cv2.resize(
        ...
        cv2.imwrite(save_dir+'/mask/temp_'+tail, mask1)  # corrosion detected
    ...
    if flag == 1:
        ...
        mask = cv2.imread(
            '/Users/sakura/Desktop/Argus 1.21/Argus_1.21_Back_end/bg_images/mask/org_' + tail, cv2.IMREAD_GRAYSCALE)
